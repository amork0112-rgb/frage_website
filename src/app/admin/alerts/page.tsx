...
export default function AdminAlertsPage() {
  ...
  const [roleCampus, setRoleCampus] = useState<string | null>(null);
  ...
  const filteredByCampus = useMemo(() => {
    return items.filter(it => !roleCampus || it.campus === roleCampus);
  }, [items, roleCampus]);
  ...
  const byLeave = useMemo(() => filteredByCampus.filter(it => it.status === "휴원 검토중"), [filteredByCampus]);
  const bySignal = useMemo(() => filteredByCampus.filter(it => it.status !== "퇴원 검토중" && it.status !== "휴원 검토중"), [filteredByCampus]);
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="space-y-3">
        {(statusTab === "퇴원 검토중" ? byQuit : statusTab === "휴원 검토중" ? byLeave : filteredByCampus).map(item => (
        ...
        {((statusTab === "퇴원 검토중" ? byQuit : statusTab === "휴원 검토중" ? byLeave : filteredByCampus).length === 0) && (
