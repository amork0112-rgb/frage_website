...
export default function DropoffMapPage() {
  ...
  useEffect(() => {
    ...
    kakaoMapScript.onload = () => {
      window.kakao.maps.load(() => {
        console.log("✅ Kakao Map SDK Loaded on DropoffMapPage");
        setKakaoReady(true);
      });
    };
  ...
  const handleSelectLocation = () => {
    if (selectedLat && selectedLng && selectedAddress) {
      localStorage.setItem("temp_dropoff_lat", selectedLat);
    ...
    <div className="flex flex-col min-h-screen bg-slate-50">
      ...
      <main className="flex-1 container mx-auto p-4 max-w-2xl flex flex-col">
        <div className="flex items-center justify-between mb-4">
          ...
          <h1 className="text-xl font-bold text-slate-900">하원 위치 선택</h1>
